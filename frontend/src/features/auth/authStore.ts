    ...
    (set) => ({
      ...
      login: async (username_or_email, password) => {
        ...
        try {
          ...
          const errorMessage = (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail || 'Error al iniciar sesión'
      ...
      register: async (email, username, password, full_name) => {
        set({ loading: true, error: null })
        try {
          const user = await authApi.register({ email, username, password, full_name })
          set({ user, isAuthenticated: true, loading: false })
        } catch (err: unknown) {
          const errorMessage = (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail || 'Error al registrar usuario'
          set({ error: errorMessage, loading: false })
          throw err
        }
      },
